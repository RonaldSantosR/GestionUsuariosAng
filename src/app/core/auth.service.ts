...
export class AuthService {
  ...
  attemptAuth(ussername: string, password: string, idAcceso:number): Observable<any> {
    ...
    return this.http.post<any>('http://localhost:8308/login/token/' + 1, null, {headers: this.headers});
  }
  ...
  attemptAuthActiveDirectory(ussername: string, password: string, idAcceso:number, objectoDominio:Dominio): Observable<any> {
    ...
    return this.http.post<any>('http://localhost:8308/login/token/' + 1, objectoDominio, {headers: this.headers});
  }
  ...
  FinAuth(): Observable<any> {
    ...
    return this.http.post<any>('http://localhost:8308/login/cerrarsession', sesion);
